...
adminOps.post("/register", async (req, res) => {
...
adminOps.post("/manager/register", async (req, res) => {
...
adminOps.post("/manager/employee/register", async (req, res) => {
...
adminOps.get("/users/active/:token", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        let result = await db
...
adminOps.get("/users/forgotPassword/:userType/:email", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        ...
        let result = await db.collection("users").findOne({email: req.params.email, userType: userType});
        if (!result) {
            res.status(404).json({
                result: false,
                status: 404,
...
adminOps.get("/users/resetPassword/:userType/:email/:token", async (req, res) => {
    try {
        ...
        let result = await db.collection("users").findOne({passwordModificationToken: token, email: email, userType: userType});
        console.log(result)
        if (! result) {
...
adminOps.post("/users/changePassword/:userType/:email", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        ...
        result = await db.collection("users").findOneAndUpdate({
            email: req.params.email, userType: userType
        }, {
            $set: {
...
adminOps.put("/users/access/:email", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        ...
        let result = await db.collection("users").findOne({email: req.params.email, userType: "standard"});
        ...
        result = await db.collection("users").findOneAndUpdate({
            email: req.params.email, userType: "standard"
        }, 
        {
            $set: {
                access: access
        ...
        res.status(200).json({message: "Access Reset Successful!", result: true, status: 200});
    } catch (err) {
        res.status(500).json({message: "Internal server error", result: false, status: 500});
    }
});

adminOps.put("/users/edit/:manager/:email", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        let manager = ObjectId(req.params.manager);
        let data = req.body;
        let result = await db.collection("users").findOne({email: req.params.email, userType: "standard", manager:manager});
        ...
        result = await db.collection("users").findOneAndUpdate({
            email: req.params.email, userType: "standard", manager: manager
        }, 
        {
            $set: {
                ...data
        ...
        res.status(200).json({message: "User Updation Successful!", result: true, status: 200});
    } catch (err) {
        console.log(err)
        res.status(500).json({message: "Internal server error", result: false, status: 500});
    }
...
adminOps.put("/users/admin/edit/:userType/:email", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        let userType = req.params.userType;
...
adminOps.post("/users/:manager", async (req, res) => {
    try {
...
adminOps.post("/login", async (req, res) => {
    try {
        let client = await mongodb.connect(dbUrl);
        let db = client.db(dbName);
        let data = await db
